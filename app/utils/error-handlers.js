...
function catchDirtyError(response, requestType, orgId, err) {
    if(err && !err.handled) {
        console.error(util.format("Failed to %s %s in gateway", requestType, orgId), errToStr(err));
...
function catchCleanError(response, requestType, orgId, err) {
    if(!err.handled) {
        console.error(util.format("Failed to %s %s in CF", requestType, orgId), errToStr(err));
